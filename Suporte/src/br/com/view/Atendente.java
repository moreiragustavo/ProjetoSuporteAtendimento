		...
		btnExcluir.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if(txtId.equals(null)) {
					JOptionPane.showMessageDialog(null, "Selecione o chamado a ser Excluido.","Erro 4000770x",
							JOptionPane.ERROR_MESSAGE);
					}
				else {
					if(JOptionPane.showConfirmDialog(null, "Você tem certeza desta ação? \nEsta Ação é Permanente "+ "e não pode ser desfeita", "ATENÇÃO", JOptionPane.YES_NO_OPTION,JOptionPane.QUESTION_MESSAGE) == 0) {
				Chamado cr = new Chamado();
				CRUDChamado cc = new CRUDChamado();
